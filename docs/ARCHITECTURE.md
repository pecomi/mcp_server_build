...
- **B6 LLM ↔ tool output**: runtime 결과 경계. host LLM이 MCP tool result를 passive data로 격리하지 않고 다음 planning context에 넣으면, 출력 텍스트가 후속 tool call을 유도할 수 있음 (RT-006).
...
│   ├── rt-003-stage1.sh         # RT-003 Stage 1 PoC (cross-server description poisoning)
│   ├── rt-006-stage1.sh         # RT-006 Stage 1 PoC (cross-server output poisoning)
...
├── research-server/             # M7 — lookup_term vehicle server (env-controllable description/output)
...
| `rt-006-stage1.sh` | research-server + host + gateway 재기동 | RT-006 — runtime output poisoning S/H 플래그 매트릭스 | 3 |
